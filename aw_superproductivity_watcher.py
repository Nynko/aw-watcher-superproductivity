...
def parse_backup_and_send():
    global last_session_end_ms, last_update_timestamp
        ...
        for project_id, days in time_tracking.items():
            ...
            for day, session in days.items():
                ...
                if start_ms and end_ms and end_ms > last_session_end_ms:
                    ...
                    associated_tasks = [t for t in tasks.values() if t.get('projectId') == project_id]
                    task_titles = ', '.join([t.get('title', 'Unnamed Task') for t in associated_tasks]) or 'Unknown Task(s)'
                    ...
                    print(f"[INFO] Sent session for {source_name} '{project_name}' with tasks [{task_titles}] starting at {timestamp_start.isoformat()} ({duration_sec}s)")
